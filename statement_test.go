...
func TestBuildStatement(t *testing.T) {
	validTests := []struct {
		...
		{"CREATE TABLE t1 (id INT64 NOT NULL) PRIMARY KEY (id)", &DdlStatements{Ddls: []string{"CREATE TABLE t1 (id INT64 NOT NULL) PRIMARY KEY (id)"}}},
		{"ALTER TABLE t1 ADD COLUMN name STRING(16) NOT NULL", &DdlStatements{Ddls: []string{"ALTER TABLE t1 ADD COLUMN name STRING(16) NOT NULL"}}},
		{"DROP TABLE t1", &DdlStatements{Ddls: []string{"DROP TABLE t1"}}},
		{"CREATE INDEX idx_name ON t1 (name DESC)", &DdlStatements{Ddls: []string{"CREATE INDEX idx_name ON t1 (name DESC)"}}},
		{"DROP INDEX idx_name", &DdlStatements{Ddls: []string{"DROP INDEX idx_name"}}},
