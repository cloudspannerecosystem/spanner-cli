...
LEFT JOIN
  INFORMATION_SCHEMA.INDEX_COLUMNS IC USING(COLUMN_NAME, TABLE_NAME)
LEFT JOIN
  INFORMATION_SCHEMA.INDEXES I USING(INDEX_NAME, TABLE_NAME)
LEFT JOIN
  INFORMATION_SCHEMA.COLUMN_OPTIONS CO USING(COLUMN_NAME, TABLE_NAME)
WHERE
  C.TABLE_NAME = '%s'
ORDER BY
